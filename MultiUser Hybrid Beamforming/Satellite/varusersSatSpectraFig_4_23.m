...
Rate_SU=zeros(1,length(Num_users)); % single user spectral efficiency (no-interference)
Rate_BS=zeros(1,length(Num_users));% analog-only beamsteering spectral efficiency
Rate_HP=zeros(1,length(Num_users)); % hybrid ZF precoding spectral efficiency
Rate_HP_MSE=zeros(1,length(Num_users)); %  hybrid MMSE precoder spectral efficiency 
Rate_HP_Kalman=zeros(1,length(Num_users)); % hybrid Kalman precoder spectral efficiency
Rate_HP_FD_ZF=zeros(1,length(Num_users)); % fully digital  ZF precoding spectral efficiency
Rate_HP_FD_MSE=zeros(1,length(Num_users)); % fully digital  MMSE precoding spectral efficiency
Rate_HP_ayash = zeros(1,length(Num_users)); %  OMP_Ayash Spectral Efficiency
Rate_HP_Yu = zeros(1,length(Num_users)); %  Novel Algorithm Spectral Efficiency 
Rate_HP_Original = zeros(1,length(Num_users)); % PE-AltMin Spectral Efficiency
...
%% --------------- Simulation starts ---------------------------------------
for useri=1:length(Num_users)
for iter=1:1:ITER
    
    % Generate user channels 
    % H is a 3-dimensional matrix, with Num_users,RX_ant,TX_ant dimensions

            [H,a_TX,a_RX]=generate_channels(Num_users(useri),TX_ant_w,TX_ant_h,RX_ant_w,RX_ant_h,Num_paths, fc, Hight, Re, Rs); 

    % Intialization of the hybrid precoder and combiner
    Frf=zeros(TX_ant,Num_users(useri)); % BS RF precoders 
    Wrf=zeros(RX_ant,Num_users(useri)); % MS RF precoders 
    
    for u=1:1:Num_users(useri)
 ...
 Ffd=He_fd'*pinv(He_fd*He_fd'); %Fully digital beamforming
    for u=1:1:Num_users(useri) % Normalization of the fully digital precoders
        Ffd(:,u)=Ffd(:,u)/sqrt((Ffd(:,u))'*(Ffd(:,u)));
    end 
    %% Spectral efficiency calculations
 
        SNR=10^(.1*SNR_dB_range)/Num_users(useri); % SNR value
 ...
 FfdMSE=inv(He_fd'*He_fd+Num_users(useri)*sigma2*eye(TX_ant))*He_fd';
 for u=1:1:Num_users(useri) % Normalization of the precoder
...
FBB_Novel = sqrt(Num_users(useri)) * FBB_Novel / norm(FRF_Novel * FBB_Novel,'fro'); %Normalization
...
FbbMSE=inv((He_fd*FRFMSE)'*(He_fd*FRFMSE)+Num_users(useri)*sigma2*FRFMSE'*FRFMSE)*(He_fd*FRFMSE)';
       
        for u=1:1:Num_users(useri) % Normalization of the hybrid precoders
...
%% Baseband zero-forcing precoding, Hybrid Beamforming 
 ...
 FRFKalman = DiscreteRFGeneration(Fopt,Steering_Vec,NtRf); %Depnding on Fully digital MMSE Fopt
  ...
  Fbbk=eye(NtRf,Num_users(useri));
  RN1=Fbbk*Fbbk';
  Qm=eye(Num_users(useri))*sigma2;
  ITERK=10; % Number of Kalman iterations
        
  for ii=1:ITERK
    Hk=He_fd*FRFKalman;
    K=RN1*Hk'*pinv(Hk*RN1*Hk'+Qm);
    errk=(eye(Num_users(useri))-Hk*Fbbk);
    errk=errk/norm(errk);
    Fbbk=Fbbk+K*errk; 
    RN=RN1-K*Hk*RN1;
    RN1=RN; 
...
for m=1:1:NtRf
    ...
    Fbb_ayash = sqrt(Num_users(useri))*Fbb_ayash/(norm(Frf_ayash*Fbb_ayash,'fro'));
...
%% AltMin Original Approach Assuming Continous Phase shifters (No Codebook)
 ...
 FBB_YuOriginal = sqrt(Num_users(useri)) * FBB_YuOriginal / norm(FRF_YuOriginal * FBB_YuOriginal,'fro');
...
%% Single User (No Interference) and Analog beamforming calculations
        for u=1:1:Num_users(useri)
            ...
            % Single-user rate
            Rate_SU(useri)=Rate_SU(useri)+log2(1+SNR*S_channel(1,1)^2)/(Num_users(useri));
            
            % Analog-only beamforming
            SINR_BS=(SNR*(abs(Wrf(:,u)'*Channel*Frf(:,u)).^2))/(SNR*sum((abs(Wrf(:,u)'*Channel*Frf(:,Int_set)).^2))+1);
            Rate_BS(useri)=Rate_BS(useri)+log2(1+SINR_BS)/(Num_users(useri));            
        end
    
        % ZF Hybrid Precoding
        Rate_HP(useri)=Rate_HP(useri)+log2(det(eye(Num_users(useri))+SNR*((He_fd*FRF_ZF)*(Fbb*Fbb')*(He_fd*FRF_ZF)')))/(Num_users(useri));
            
        % Hybrid Precoding MMSE
         Rate_HP_MSE(useri)=Rate_HP_MSE(useri)+log2(det(eye(Num_users(useri))+SNR*((He_fd*FRFMSE)*(FbbMSE*FbbMSE')*(He_fd*FRFMSE)')))/(Num_users(useri));

        % New Algorithm
         Rate_HP_Yu(useri)=Rate_HP_Yu(useri)+log2(det(eye(Num_users(useri))+SNR*(He_fd*(F_Novel*F_Novel')*He_fd')))/(Num_users(useri));

          % AltMin Algorithm
         Rate_HP_Original(useri)=Rate_HP_Original(useri)+log2(det(eye(Num_users(useri))+SNR*(He_fd*(F_YuOriginal*F_YuOriginal')*He_fd')))/(Num_users(useri));

         
        % Hybrid Precoding Kalman
        Rate_HP_Kalman(useri)=Rate_HP_Kalman(useri)+log2(det(eye(Num_users(useri))+SNR*(He_fd*(Fkalman*Fkalman')*He_fd')))/(Num_users(useri));

        % ZF fully digital precoding
        Rate_HP_FD_ZF(useri)=Rate_HP_FD_ZF(useri)+log2(det(eye(Num_users(useri))+SNR*(He_fd*(Ffd*Ffd')*He_fd')))/(Num_users(useri));
        
        % MSE fully digital precoding
        Rate_HP_FD_MSE(useri)=Rate_HP_FD_MSE(useri)+log2(det(eye(Num_users(useri))+SNR*(He_fd*(FfdMSE*FfdMSE')*He_fd')))/(Num_users(useri));
        
        % OMP Algorithm 
        Rate_HP_ayash(useri)=Rate_HP_ayash(useri)+log2(det(eye(Num_users(useri))+SNR*(He_fd*(F_ayash*F_ayash')*He_fd')))/(Num_users(useri));
...
%Plotting the spectral efficiency
    figure,plot(Num_users,Rate_SU,'-mv','LineWidth',1.5); hold on;
        hold on; plot(Num_users,Rate_HP_FD_MSE,'--k','LineWidth',2);
        hold on; plot(Num_users,Rate_HP_Original,'->','Color',[0.5 0 0.8],'LineWidth',2);

                hold on; plot(Num_users,Rate_HP_Yu,'-^g','LineWidth',2);
    hold on; plot(Num_users,Rate_HP_ayash,'-+c','LineWidth',2);
    hold on; plot(Num_users,Rate_HP_Kalman,'-yo','LineWidth',2);
    hold on; plot(Num_users,Rate_HP_MSE,'-b*','LineWidth',1.5);
    plot(Num_users,Rate_HP,'-rs','LineWidth',1.5);
    ...
    hold on; plot(Num_users,Rate_BS,'-ko','LineWidth',1.5);
...
%% Fully Digital MMSE is underperforming because the effective channel is poorly conditioned, it is being close to singular (its determinate is zero), it is not invertible because many eigen values are close to zero and the determinate is the multiplication of these eigen values
