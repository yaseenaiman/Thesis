function SP = top_sim_function_com(SP)
...
%         y_quant = quantization(y_vec, SP.ADCbits, max(abs([real(y_vec);imag(y_vec)])));
 ...
 m1 = size(PhiPsi);
    m=m1(2); 
        ...
        [x_hat_OMP_author] = OMPa(PhiPsi, y_quant, iterations); 
...
x_hat_NOMP = x_ACGP;
 ...
 disp('Starting OMP by Yaseen')
  [x_OMP] = omp_yas(y_quant, PhiPsi, iterations); 
 x_hat_OMP = x_OMP;
...
% Plotting the rates
figure, plot(SP.SNR_db_array ,SP.LS,'b','LineWidth',1.5);
